...
export class GestionObservacionesComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.observacionService.findByFilter(filterQuery).then(
      resp => this.observacion = resp['data'][0]
    ).then(
      resp => {
  ...
  loadCausaRaiz() {
    if (this.causaRaizList == null) {
      this.sistemaCausaRaizService.findDefault().then(
        data => this.causaRaizList = this.buildTreeNode((<SistemaCausaRaiz>data).causaRaizList, null)
      );
    }
  }
  ...
  buildTreeNode(crList: CausaRaiz[], parentNode: any): any {
    ...
    crList.forEach(cr => {
      ...
      node.children = (cr.causaRaizList == null || cr.causaRaizList.length == 0 ? null : this.buildTreeNode(cr.causaRaizList, node));
  ...
  adicionarSeleccionados(node: any) {
    for (let i = 0; i < this.observacion.causaRaizAprobadaList.length; i++) {
  ...
  guardarDenegar() {
    this.observacion.motivo = this.motivo;
    this.observacionService.denegarObservacion(this.observacion).then(
      data => {
        this.visibleObservacion = false;
