import { Component, OnInit, ViewChild, HostListener, Input, Output, EventEmitter, ElementRef } from '@angular/core';

@Component({
  selector: 's-panelFlotante,[s-panelFlotante]',
  template: '<ng-content></ng-content>'
})
export class PanelFlotanteComponent implements OnInit {
  @Input('sticky') sticky: string;
  @Input('zIndex') zIndex: number = 10;
  @Input('width') width: string = 'auto';
  @Input('offset-top') offsetTop: number = 0;
  @Input('offset-bottom') offsetBottom: number = 0;
  @Input('start') start: number = 0;
  @Input('class') stickClass: string = 'sticky';
  @Input('end-class') endStickClass: string = 'sticky-end';
  @Input('media-query') mediaQuery: string = '';
  @Input('parent') parentMode: boolean = true;
  @Input('orientation') orientation: string = 'none';
